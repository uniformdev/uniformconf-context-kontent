...
export const Talk = ({ elements: { title, description, audience } }: TalkProps) => (
  <div className="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow space-y-2 pt-2">
    <div className="flex-none mt-auto bg-white rounded-b rounded-t-none overflow-hidden">
      <div className="mt-3 mb-3 flex items-center justify-start">
        <AudienceLabel audienceName={audience.value[0].name} />
      </div>
    </div>
    <a href="#" className="flex flex-wrap no-underline hover:no-underline">
      <div className="w-full font-bold text-xl text-gray-800 px-6">{title.value}</div>
    </a>
    <KontentRichText className="text-gray-800 px-6 pb-6 text-sm" field={description} />
  </div>
);
